import {getFeaturedProjects, getProjectBySlug} from "@/lib/projects/projects-select";
import {notFound} from "next/navigation";
import Link from "next/link";
import {ArrowLeftIcon, CalendarIcon, HeartIcon, StarIcon, User2Icon} from "lucide-react";
    ...
    return (
        <section className='flex flex-col gap-2 py-10 px-10'>
            <div className='flex justify-between'>
                <Link href='/explore' className='flex gap-2 hover:underline hover:text-blue-800'>
            ...
            <div className='mt-8'>
                <SectionHeading title={project.name} description={project?.github_url} icon={StarIcon}/>
                        ...
                        <div className='flex flex-col gap-2 mt-5'>
                            ...
                            <div className='flex gap-2'>
                                ...
                                {project?.techStack?.map((techStack) => (
                                    <Badge key={techStack} className='bg-green-900 px-4 py-1.5'>{techStack}</Badge>
                                ))}
                            </div>
                        </div>
                        <div>
                            <span className='font-semibold'>Description</span>
                            <div className='bg-[#f3f3f3] lg:w-[800px] sm:w-[300px] px-4 py-1.5 h-[140px] rounded-md'>
                                <p className='text-slate-700 font-light'>{project.description}</p>
                            </div>
                        </div>
                        <div>
                            <span className='font-semibold mb-2'>Additional Info</span>
                            <div className='bg-[#f3f3f3] lg:w-[800px] sm:w-[300px] px-4 py-1.5'>
                                <User2Icon className='inline-block w-5 mr-2' />
                                <span className='font-semibold'>Published By</span>
                                <p className='text-slate-700 font-light'>{project.submittedBy}</p>
                                <hr className='border-b-2 border-gray-200 mb-4' />
                                <CalendarIcon className='inline-block w-5 h-5 mr-2' />
                                <span className='font-semibold inline-block mr-2'>Published On: </span>
